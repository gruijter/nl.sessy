...
class Sessy {
  ...
  // target: 'OTA_TARGET_SELF' (dongle) or 'OTA_TARGET_SERIAL' (attached battery/meter)
  async installOTA(opts) {
    try {
      const options = opts || {};
      const data = { target: options.target };
      const res = await this._makeRequest(setUpdateEP, data);
      return Promise.resolve(res);
    } catch (error) {
      return Promise.reject(error);
    }
  ...
  // Known /api/v1/system/settings fields (GET returns all of them, POST merges any subset):
  //   pv_hostname            string    hostname/IP of the linked Sessy battery (set on P1/CT dongle)
  //   p1_hostname            string    hostname/IP of the linked P1 dongle (set on Sessy/CT device)
  //   active_phase           number    grid phase (1-3) this device is wired to
  //   group_current          number    max current (A) allowed for the whole install group
  //   phase_current          number    max current (A) allowed per phase
  //   group_sessys           number    number of Sessy batteries in this group
  //   phase_sessys           number    number of Sessy batteries on this phase
  //   total_sessys           number    total number of Sessy batteries in the install
  //   min_power              number    lower bound (W) for power setpoint/strategies, observed range 50-2000
  //   max_power              number    upper bound (W) for power setpoint/strategies, observed range 50-2200
  //   disable_noise_level    boolean   true disables the noise limit entirely (allowed_noise_level is ignored)
  //   allowed_noise_level    number    max allowed noise level 1-5 (only enforced when disable_noise_level is false)
  //   enabled_time           string    daily operating window "HH:MM-HH:MM", outside of it the battery is idle
  //   cloud_enabled          boolean   whether the device reports to the Sessy cloud
  //   sessy_enabled          boolean   master on/off switch for the battery
  //   eco_charge_hours       number    0-24, hours per day reserved for eco (cheap-window) charging (fw 1.6.8+)
  //   eco_charge_power       number    charge power (W) used during eco charging, e.g. 50-2200 (fw 1.6.8+)
  //   eco_nom_charge         boolean   let NOM strategy also charge during the eco window (fw 1.6.8+)
  //   pack_temp_limit_enabled boolean  "Temperatuursafhankelijke vermogensbegrenzing": derate power based on
  //                                    battery pack temperature (fw 1.9.0+). Not seen in a live response yet,
  //                                    reported by the ha-sessy integration - verify on-device before relying on it.
  //   min_soc                number   0-100, minimum state of charge the battery won't discharge below.
  //                                    Not seen in a live response yet, reported by the ha-sessy integration -
  //                                    verify on-device before relying on it.
  // (unrelated to this endpoint: power strategy/setpoint live under /api/v1/power/*, see getStrategy()/setStrategy())
