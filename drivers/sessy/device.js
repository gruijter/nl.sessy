  ...
  async updateDeviceState(status, strategy, energy, systemSettings) {
    ...
    try {
      ...
      const capabilityStates = {
        ...
        'measure_current.inverter': Number.isFinite(status.sessy.inverter_current_ma) ? status.sessy.inverter_current_ma / 1000 : undefined,
        'measure_voltage.pack': Number.isFinite(status.sessy.pack_voltage) ? status.sessy.pack_voltage / 1000 : undefined,
        'measure_power.external': Number.isFinite(status.sessy.external_power) ? status.sessy.external_power : undefined,
